...
function startApp(argv){
    ...
    var port = argv.param('p','port') || 8000;
    var debug = argv.mode('debug') !== null ? argv.mode('debug') : false;
    var dir = argv.param('d', 'dir') || '.swarm';
    
    Swarm.env.debug = debug;
    ...
    // use file storage
    var fileStorage = new Swarm.FileStorage(dir);
...
var program = require('electron')('./TodoAppServer.js');

program
    .name('Server runner for ' + pkg.name)
    .version(pkg.version)
    .command('default')
    .option('-p, --port [8000]', 'port for listening')
    .option('-d, --dir [.swarm]', 'FileStorage dir')
    .option('--debug', 'mode')
    .action(startApp)
